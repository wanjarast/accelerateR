...
```{r , eval = F}
model.svm <- svm(x = train_rescaled[ , -c(1,2)] , y = train_rescaled$Bev , type = "C-classification" , kernel = "radial" , class.weights = behaviour_weights)
```
...
```{r}
val_predict <- predict(object = model.svm , newdata = val_rescaled[ , -c(1,2)])
